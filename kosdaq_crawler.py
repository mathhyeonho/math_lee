...
//
class kosdaq_crawler:
    def __init__(self):
        self.address = 'https://finance.naver.com/sise/sise_market_sum.nhn?sosok=1'
        self.data = []
    ...
    def wwww(self):
        ...
        with open('/workspace/jupyter_notebook/copylist/{0}_{1}_{2}_kosdaq.csv'.format(today_list[0], today_list[1], today_list[2]), 'a') as file:
            file.write('point;price;per;percentage_of_foreign\n')
            for i in self.data:
                file.write('{0};{1};{2};{3}\n'.format(i[0], i[1], i[2], i[3]))

kosdaq_instance = kosdaq_crawler()
kosdaq_instance.copy(1, 13)
kosdaq_instance.copy(3, 14)
kosdaq_instance.copy(3, 14)
kosdaq_instance.wwww()

import pandas as pd
import matplotlib as mpl
import matplotlib .pyplot as plt

df = pd.read_csv('/workspace/jupyter_notebook/copylist/{0}_{1}_{2}_kosdaq.csv'.format(today_list[0], today_list[1], today_list[2]), index_col = 'point', sep = ';', thousands = ',')
...
df.sorted_by_values.to_csv('/workspace/jupyter_notebook/copylist/{0}_{1}_{2}_kosdaq_sorted_by_per.csv'.format(today_list[0], today_list[1], today_list[2]))
